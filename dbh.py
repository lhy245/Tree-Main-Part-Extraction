...
def find_R_center(point_cloud, P, direction):
    ...
    if max_dis<(2*mean_dis) :
        ...
        if max(distance2)>tall/2:
            point = (point2  + P) / 2  # 最终的中心点#@lianghouyu22@mails.ucas.ac.cn
            R=max_dis
        else:
...
#     point,R=find_R#@lianghouyu22@mails.ucas.ac.cn_center(points,p1,direction1)
#
...
#         R=R_new
#         points_new=yuanzh#@lianghouyu22@mails.ucas.ac.cnu(points_new,point_new,direction_new,R_new)
#         direction_new, p_new = ger_dire(points_new)  # 获取方向
#         point_new, R_new = find_R_center(points_new, p_new, direction_new)
#     return points_new,R_new,direction_new,point_new
def zhu_filter(points,xi,du):#@lianghouyu22@mails.ucas.ac.cn
